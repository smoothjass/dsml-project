...
def kNN_Regression(X_train, y_train, X_test):
    ...
    # hyperparameters
    k = list(range(1, 10)) # all options for k that should be tried
    parameter_grid = {
        'n_neighbors': k,  # Possible values for the number of neighbors
        'weights': ['uniform', 'distance'],  # Possible values for the weight function (uniform: all the same weight, distance: closer ones have higher influence)
    ...
    grid_search = GridSearchCV(regressor, parameter_grid, cv=5, scoring='neg_mean_squared_error') # scoring chosen to minimize mse
    # 5-fold cross validation of data bc:
    # - reasonable computational cost,
    # - only 20% test-data (80% train-data) used
    # - lower variance - more stable estimate of performance
