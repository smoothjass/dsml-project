...
def explore_and_clean(city_data):
    ...
    city_data = remove_outliers(7, 'cleanliness_rating', city_data)
    city_data = remove_outliers(7, 'rest_index', city_data)
    city_data = remove_outliers(7, 'attr_index', city_data)
    city_data = remove_outliers(7, 'metro_dist', city_data)

    # print boxplot again to see how outliers have changed
    # city_data.plot(kind="box", subplots=True, layout=(4, 3), figsize=(30, 30))
    # plot.show()

    # other columns also have values that are more than 7 standard deviations away from the mean
    ...
    # # plot average price for different satisfaction ratings [from 70 to 100 in steps of 5]
    # plt.bar([str(i) + '-' + str(i + 5) for i in range(70, 100, 5)], [city_data[city_data['guest_satisfaction_overall'] == i]['realSum'].mean() for i in range(70, 100, 5)])
    # plt.ylabel('Average price')
    # plt.title("Average price for different satisfaction ratings")
    # plt.show()

    # # plot percentage of superhosts for different prices [from 0 to 1000 in steps of 100]
    # plt.bar([str(i) + '-' + str(i + 100) for i in range(0, 1000, 100)], [city_data[city_data['realSum'] >= i]['host_is_superhost'].mean() for i in range(0, 1000, 100)])
    # plt.ylabel('Percentage of superhosts')
    # plt.xlabel('Price')
    # plt.title("Percentage of superhosts for different prices")
    # # increase spacing between xticks
    # plt.xticks(rotation=45)
    # # increase height of plot because labels are cut off
    # plt.gcf().subplots_adjust(bottom=0.2)
    # plt.show()
    #

    # # plot metro_dist vs price
    # plt.scatter(city_data['metro_dist'], city_data['realSum'])
    # plt.ylabel('Price')
    # plt.xlabel('Distance to metro')
    # plt.title("Distance to metro vs price")
    # # fit a linear regression line
    # plt.plot(np.unique(city_data['metro_dist']),
    #             np.poly1d(np.polyfit(city_data['metro_dist'], city_data['realSum'], 1))(np.unique(city_data['metro_dist'])),
    #             color='red')
    # plt.show()

    # # plot attr_index vs price
    # plt.scatter(city_data['attr_index'], city_data['realSum'])
    # plt.ylabel('Price')
    # plt.xlabel('Attraction index')
    # plt.title("Attraction index vs price")
    # # fit a linear regression line
    # plt.plot(np.unique(city_data['attr_index']),
    #             np.poly1d(np.polyfit(city_data['attr_index'], city_data['realSum'], 1))(np.unique(city_data['attr_index'])),
    #             color='red')
    # plt.show()
    #

    # plot person_capacity vs price
    # plt.bar(['1 person', '2 people', '3 people', '4 people', '5 people', '6 people'],
    #         [city_data[city_data['person_capacity'] == 1]['realSum'].mean(),
    #             city_data[city_data['person_capacity'] == 2]['realSum'].mean(),
    #             city_data[city_data['person_capacity'] == 3]['realSum'].mean(),
    #             city_data[city_data['person_capacity'] == 4]['realSum'].mean(),
    #             city_data[city_data['person_capacity'] == 5]['realSum'].mean(),
    #             city_data[city_data['person_capacity'] == 6]['realSum'].mean()])
    # plt.ylabel('Average price')
    # plt.title("Average price for different number of people")
    # plt.show()
